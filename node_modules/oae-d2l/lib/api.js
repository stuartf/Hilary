...
var isAuthed = module.exports.isAuthed = function(ctx, redirect, callback) {
    ...
    _getUserSession(ctx, function(err, session) {
        if (!err && session) {
            // Try getting whoami to make sure the session works
...
var submitToD2L = module.exports.submitToD2L = function(ctx, contentId, courseId, assignmentId, description, callback) {
    ...
    ContentAPI.getFullContentProfile(ctx, contentId, function(err, contentProfile) {
        if (err) {
            return callback(err);
        }
        ...
        if (contentProfile.resourceSubType === 'file') {
            ...
            request.get(contentPath, function(err, response, body) {
                ...
                _getSignedUrl(ctx, d2lUrl, 'POST', function(err, signedUrl) {
                    ...
                    parts.push({
                        'disposition': 'form-data; name=""; filename="' + contentProfile.filename + '"',
                        'type': contentProfile.mime,
                        'body': fs.readFileSync(stream.path)
                    });

                    _uploadMultipartMixed(signedUrl, parts, function(err) {
                        if (err) {
                            return callback(err);
                        }
                        _saveSubmission(ctx, contentProfile, courseId, assignmentId, function(err) {
                            if (err) {
                                return callback(err);
                            }
            ...
            _getSignedUrl(ctx, d2lUrl, 'POST', function(err, signedUrl) {
                ...
                parts.push({
                    'disposition': 'form-data; name=""; filename="' + contentProfile.displayName + '.html"',
                    'type': 'text/html',
                    'body': contentProfile.latestRevision.etherpadHtml
                });
            ...
            return callback({'code': 400, 'msg': 'Unsupported content type for d2l upload'});
        }
    });
};
 ...
 *
 */
var _saveSubmission = function(ctx, contentProfile, courseId, assignmentId, callback) {
    D2LDAO.getD2LSubmissions(contentProfile.id, ctx.user().id, function(err, d2lSubmissions) {
        if (err) {
            return callback(err);
        }
        ...
        D2LDAO.saveD2LSubmissions(contentProfile.id, ctx.user().id, d2lSubmissions, function(err) {
            if (err) {
                return callback(err);
            }
            callback(null);
...
/**
 * Simple mime multipart/mixed upload implementation, probably not complete, but handles the cases we need
 */
var _uploadMultipartMixed = function(uploadUrl, parts, callback) {
    var options = url.parse(uploadUrl, true);
    var boundary = 'fd88feef9d6e03551e4d7aa4b6a48f00';
    options.method = 'POST';

    var newLine = new Buffer('\r\n');
    var boundaryBuffer = new Buffer('--' + boundary);

    var msg = new Buffer('');

    _.each(parts, function(part, index) {
        if (index > 0) {
            msg = Buffer.concat([msg, newLine]);
        }
        msg = Buffer.concat([msg, boundaryBuffer]);

        if (part.disposition) {
            msg = Buffer.concat([msg, newLine, new Buffer('Content-Disposition: ' + part.disposition)]);
        }

        var body = Buffer.isBuffer(part.body)? part.body : new Buffer(part.body);
        msg = Buffer.concat([msg, newLine, new Buffer('Content-Type: ' + part.type), newLine, newLine, body]);
    });

    msg = Buffer.concat([msg, newLine, boundaryBuffer, new Buffer('--'), newLine]);
    ...
    var myhttp = (options.protocol === 'https://' ? https : http);
...
/**
 * Get all the feedback this user has received on this content in d2l
 */
