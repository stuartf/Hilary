...
var getD2LFeedback = module.exports.getD2LFeedback = function(ctx, contentId, callback) {
    D2LDAO.getD2LContent(contentId, ctx.user().id, function(err, d2lSubmissions) {
        if (err) {
            return callback(err);
        }
        ...
        _getSignedUrl(ctx, whoamiUrl, 'GET', function(err, signedUrl) {
            ...
            request(signedUrl, function(err, response, body) {
                ...
                _.each(d2lSubmissions, function(submission) {
                    var courseId = submission.courseId;
                    var assignmentId = submission.assignmentId;
                    feedback[courseId] = feedback[courseId] || {};
                    feedback[courseId][assignmentId] = {'timestamp': submission.timestamp};
                    var d2lUrl = _getD2LAPIPrefix('le') + '/' + courseId + '/dropbox/folders/' + assignmentId + '/feedback/user/' + entityId;
                    _getSignedUrl(ctx, d2lUrl, 'GET', function(err, signedUrl) {
                        if (err) {
                            return callback(err);
                        }
                        request(signedUrl, function(err, response, body) {
                            ...
                            feedback[courseId][assignmentId] = JSON.parse(body);
                        });
                    });
                });
